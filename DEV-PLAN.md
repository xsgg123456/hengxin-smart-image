...
## 2026-09-24 · 标注功能提交与生产发布

1. 提交已批准功能快照；只纳入本轮文件，保留无关性能分析文档。
2. 升级前端0.2.5，适配既有发布脚本的生产overlay链及0017无迁移流程；构建、白名单打包、隐私与依赖审计，独立审查发布增量后提交。
3. 校验生产当前版本及任务为空，在服务器构建固定镜像并从安装镜像跑隔离专项测试；停止接纳、排空、备份后切换API/Worker和前端。
4. 验证部署文件哈希、镜像ID、0017、队列心跳、公开健康及匿名登录页；记录发布及回退信息，提交发布文档。

## 2026-09-24 · 单画布标注正式接入

实施与本地验收记录：[正式标注验证](docs/INLINE-ANNOTATION-VALIDATION-20260924.md)。前端170项、后端985项通过，170项环境相关跳过；类型、生产/Demo构建及隔离真实页面两入口测试通过。生产发布单独授权。

1. 公共编辑器：承接最终预览的矩形、画笔、固定拖动手柄和编号意见；真实原图加载、原尺寸合成、预览确认、草稿隔离。验收缩放后连续标注/平移、撤销保留意见及非方形原尺寸导出。
2. 业务入口：API/CLI/Demo 接入同一编辑器，保留已有上传和整套流程；验收指定版本、失效加载、提交锁定、版本冲突及不确定重试的文件/文本/键一致。
3. 提示词：完善矩形、画笔、编号的定位语义；兼容手工上传，无标注调用不变，API 冻结快照继续复用，CLI 商品/文字同样识别标注。
4. 运行前后端回归、类型检查、构建和隔离浏览器流程，记录接口模拟与真实验证边界，独立两阶段审查并登记快照。此轮不部署生产。

2026-09-24最新实施：单画布仅框选/画笔工具选择，移除平移模式，新增画布内拖动手柄（放大可拖，原比例提示先放大）；验证两个工具选中时都可直接拖动图片再继续标注，不依赖快捷键。保留历史标注和版本，独立审查本增量。

2026-09-24预览交互合并：增加临时平移、当前手势提示与焦点/指针取消处理，验收放大后的“框选→临时平移→继续框选”和“画笔→临时平移→继续画笔”，不误新增标注、不影响意见输入和既有按钮。仅预览。

2026-09-24预览增量：添加画笔路径、每笔编号意见及撤销/删除/导出，保持已有缩放和平移逻辑，完成浏览器混合标注与既有交互回归、独立审查。

## 2026-09-24 · 单张修改直接标注预览

仅制作隔离交互预览，依次完成现有视觉复用、图片坐标标注和意见编辑、自动合成提交预览、浏览器验证与独立审查。按docs/INLINE-ANNOTATION-PREVIEW-20260924.md验收，不接生产API；正式开发需用户确认。

## 2026-09-23 · 图片整改与多图修正生产发布

1. 提交已审三项整改，发布版本0.2.4，复核线上镜像/0016迁移/在途任务，完成白名单构建、依赖与隐私审计。
2. 审查发布脚本和版本增量，固定快照提交；归档代码/迁移/前端静态产物，哈希核对后上传。原有在途任务排空、生产数据库/前端/两份CLI源码备份，0017只增列、不做降级。
3. 切换API/独立APIworker及相关outbox镜像；原生CLI受控更新prompts.py和image_revision_prompt.py，验证消费、心跳；前端保留旧哈希文件最后切换首页。恢复旧服务的回退不得覆盖新业务数据、删除快照或强停在途任务。
4. 核对安装产物哈希、schema0017、健康/鉴权/登录页并记录实际发布；没有真实登录态时明确业务线上交互尚未实测，不绕过认证。
