async (page) => {
  ...
  await page.route('**/api/v1/**', route => {
    ...
    const body = path.endsWith('/auth/me') ? user : scenario === 'malformed' ? { templates: [null], tasks: [], archives: [] } : { templates: [], tasks: [], archives: [] };
  ...
  await page.getByText('暂无可用模板，请先创建模板', { exact: true }).waitFor();
