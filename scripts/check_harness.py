...
def main():
    ...
    assert sum(len(group["hooks"]) for groups in hooks.values() for group in groups) == 5
    ...
    print(f"PASS: {len(found)} skills, 2 agents, 5 hooks, {len(manifest['files'])} upstream files present", flush=True)
