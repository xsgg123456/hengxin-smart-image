...
[总体规则]
    - Git 提交信息必须使用中文：提交标题的描述和正文均用中文，适用于普通提交、合并提交和 squash 提交。可保留 feat/fix/docs/chore 等 Conventional Commits 类型、scope、代码标识符和专有名词；例如 docs: 规定 Git 提交信息使用中文。提交前检查，不使用纯英文描述。
...
[Sub-Agent 调度规则]
    ...
    中途问答、等待用户或明确暂停可直接结束回复，不要求 review-checkpoint；结束回复不是验收通过，不允许据此提交未审代码或宣称完成。提交前仍检查审查凭据与类型，完成交付必须匹配已批准快照。
