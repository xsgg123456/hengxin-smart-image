...
from .models import ACTIVE, RoundRecord, TaskRecord, TaskRequest, TaskSource, ResultSlotRecord
...
def enqueue(session, user, task, note, target=None):
    ...
    round = RoundRecord(id=round_id, task_id=task.id, job_id=job_id, operator_id=user.id,
        note=note, target=target, status='queued', execution_config={
            'version': 1, 'concurrency': settings.generation_concurrency})
    session.add(round)
    session.flush()
...
def create_task(session, user, body, key):
    ...
    task = TaskRecord(name=body.name.strip(), sku=(body.sku or '').strip(), mode=body.mode,
        ...
        skill_snapshot={'id': str(skill.id), 'version': skill.version, 'checksum': skill.checksum,
                        'name': skill.skill.name}, execution_source='fixture')
    session.add(task)
    session.flush()
...
def accept_round(session, user, task_id, body, key):
    """Internal Phase8 contention seam; HTTP revisions remain Phase10."""
    authorize(user)
    ...
    if previous:
        return previous
    if session.scalar(select(RoundRecord.id).where(RoundRecord.task_id == task_id,
                                                   RoundRecord.status.in_(ACTIVE))):
        raise HTTPException(409, '任务仍有未结束轮次，请保留意见并等待')
    if not body.note.strip() or len(body.note) > 1000:
