...
def test_wallpaper_brief_matches_manual_format_with_all_actual_images():
    ...
    assert prompt.startswith(f"使用 {data['skillPath']} 所定义的技能。\n\n请读取本次任务图片：")
    assert '待修改底图（共 4 张）' in prompt and '手机屏幕素材（共 1 张）' in prompt
    assert '把这张手机屏幕素材替换掉图片的壁纸与镜头，其他一律不允许有任何改变。' in prompt
    brief, protocol = prompt.split('后台结果交付规则：')
    for item in data['inputs'] + data['targets']:
        assert item['path'] in brief
    assert 'manifest.json' in protocol and '本次补充要求' not in prompt
    assert json.loads(protocol.split('本轮完整图片与结果槽映射（JSON 数据）：\n')[1]) == data
...
def test_rework_keeps_current_image_original_target_and_local_slot_zero():
    ...
    assert '"slot": 0, "taskSlot": 3' in prompt
...
def test_multiple_inputs_and_rework_without_successful_current_result():
    ...
    assert '/work/inputs/01.webp' in prompt and '没有当前结果的槽使用原底图' in prompt
