<template>
  <div class="hx-page">
    ...
    <template v-if="report">
      ...
      <div class="hx-stats hx-gap"><ElCard class="art-card"><span>总体服务</span><strong>{{ states[report.state] }}</strong></ElCard><ElCard class="art-card"><span>排队任务</span><strong>{{ monitorCount(report.queueSize) }}</strong></ElCard><ElCard class="art-card"><span>正在执行</span><strong>{{ monitorCount(report.runningCount) }}</strong></ElCard><ElCard class="art-card"><span>最后检查</span><strong style="font-size: 15px">{{ report.checkedAt ? time(report.checkedAt) : '未检查' }}</strong></ElCard></div>
