<template>
  <div class="hx-page"><div class="hx-heading"><div><span class="hx-eyebrow">SYSTEM SETTINGS</span><h1>系统配置</h1><p>配置仅影响后续轮次，保存记录版本和实际操作者。</p></div><ElButton :loading="loading" :disabled="saving" @click="load">重新读取</ElButton></div><AdminPreview />
    ...
    <div v-if="form && data" class="hx-stack">
      ...
      </ElForm><ElButton class="hx-gap" type="primary" :loading="saving" :disabled="loading || !!error || data.timeoutCapacity < 60" @click="save">保存配置</ElButton></ElCard>
...
const {form,uploadMiB,saving,saveError,save: persist}=useSettingsEditor({data,loading,error,load},saveSettings)
