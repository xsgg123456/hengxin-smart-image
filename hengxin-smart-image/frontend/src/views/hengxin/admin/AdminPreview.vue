<template>
  <template v-if="isMockMode">
    <ElAlert title="前端模拟预览 · 统计、健康、部署检查及配置均为演示数据，未连接真实后台。" type="warning" :closable="false" show-icon />
...
const scenarios = [{ value: 'default', label: '常规预览' }, { value: 'empty', label: '空数据' }, { value: 'list-error', label: '读取失败与重试' }, { value: 'save-error', label: '保存失败与重试' }, { value: 'install-error', label: '部署检查失败保留旧版' }, { value: 'idle', label: '执行器空闲' }, { value: 'unknown', label: '健康检查过期' }, { value: 'unavailable', label: '存储不可达' }, { value: 'worker-lost', label: 'Worker 失联' }, { value: 'auth-rejected', label: 'CLI 认证拒绝' }, { value: 'rate-limited', label: '执行限流' }, { value: 'timeout', label: '执行超时' }]
