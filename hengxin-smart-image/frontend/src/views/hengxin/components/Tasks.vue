<template>
  <div class="hx-page">
    <div class="hx-heading"><div><span class="hx-eyebrow">YOUR CREATIVE PIPELINE</span><h1>任务中心</h1><p>从提交到成品，每一套图片的进度都在这里。</p></div><ElButton type="primary" @click="router.push('/image-processing/wallpaper')"><ArtSvgIcon icon="ri:add-line" /> 新建套图任务</ElButton></div>
    ...
    <ElCard class="art-card hx-section" shadow="never"><div class="hx-filter"><ElRadioGroup v-model="status"><ElRadioButton value="全部">全部任务</ElRadioButton><ElRadioButton value="处理中">处理中</ElRadioButton><ElRadioButton value="待查看">待查看</ElRadioButton><ElRadioButton value="失败">异常</ElRadioButton></ElRadioGroup><ElInput v-model="search" clearable placeholder="搜索任务名称或编号" class="hx-search"><template #prefix><ArtSvgIcon icon="ri:search-line" /></template></ElInput></div>
      <ArtTable :data="filtered" :columns="columns" row-key="id" empty-text="没有找到任务，试试其他筛选条件" :show-pagination="false">
        ...
        <template #progress="{ row }"><ElProgress v-if="row.progress !== null" :percentage="row.progress" :stroke-width="5" /><span v-else>{{ row.state }}</span><small class="hx-muted">{{ row.images.length }} 张图片</small></template>
      ...
      <p class="hx-footnote">共 {{ filtered.length }} 个任务</p>
    </ElCard>
...
const router = useRouter(), route = useRoute(), search = ref(''), status = ref('全部'), detailOpen = ref(false), currentId = ref('')
const current = computed(() => db.tasks.find(task => task.id === currentId.value))
