    ...
    <div class="hx-create-grid">
      <div class="hx-stack">
        ...
        <ElCard class="art-card hx-section" shadow="never">
          <div class="hx-section-title"><h2><span class="hx-number">{{ mode === 'text' ? '01' : '02' }}</span>{{ mode === 'text' ? '上传待修改图片' : '上传替换素材' }}</h2><ElButton v-if="isMockMode" text type="primary" @click="useExample">使用示例素材</ElButton></div>
          <ElUpload drag multiple accept="image/png,image/jpeg,image/webp" :auto-upload="false" :show-file-list="false" :on-change="onUpload">
            <div class="hx-upload-icon"><ArtSvgIcon icon="ri:upload-cloud-2-line" /></div><strong>点击上传，或将图片拖到这里</strong><p>{{ mode === 'wallpaper' ? '上传希望放入手机屏幕的新壁纸' : mode === 'product' ? '上传希望替换到模板中的商品图片' : '上传需要替换文字的商品图片' }}</p><small>JPG / PNG / WebP · 预览单张上限 10 MiB</small>
          </ElUpload>
      ...
      <aside class="hx-summary">
        <ElCard class="art-card hx-summary-card" shadow="never"><div class="hx-section-title"><h2>本次任务</h2><ElTag effect="light" round>新任务</ElTag></div>
          ...
          <div class="hx-skill-note"><ArtSvgIcon icon="ri:sparkling-2-line" /><div><strong>{{ skills[mode] }}</strong><small>{{ isMockMode ? '模拟绑定 · 交互演示' : '由系统自动调用' }}</small></div></div>
          <ElButton type="primary" size="large" class="hx-full" :loading="submitting" @click="submit"><ArtSvgIcon icon="ri:sparkling-line" /> 提交生成任务</ElButton><p class="hx-footnote">{{ isMockMode ? '生成返回示例图片，仅用于前端预览。' : '任务受理后可在任务中心查看状态。' }}</p>
        </ElCard>
...
import { db, labels, skills, loadExamples, readPictures, createTask, type Mode, type Picture } from '../model'
...
async function useExample() { sources.value = await loadExamples(props.mode, props.mode === 'text' ? 2 : 1); if (!name.value) name.value = props.mode === 'text' ? '秋日上新 · 文案更新' : props.mode === 'product' ? '新品钢化膜 · 商品主图' : '秋日山川 · 手机屏幕套图'; if (props.mode === 'text') note.value = '将「新品上市」改为「秋日上新」，其他内容保持不变'; error.value = '' }
...
async function submit() {
  ...
  error.value = !sources.value.length ? (isMockMode ? '请先上传素材或使用示例素材' : '请先上传素材') : !name.value.trim() ? '请填写任务名称' : props.mode !== 'text' && !template.value ? '请选择可用模板' : props.mode === 'text' && !note.value.trim() ? '请填写文字修改要求' : ''
  if (error.value) { ElMessage.warning(error.value); return }
  submitting.value = true
  try {
    const accepted = await createTask({ mode: props.mode, name: name.value.trim(), templateId: template.value?.id, sources: sources.value, note: note.value.trim() })
    await router.push({ path: '/tasks/index', query: { task: accepted.taskId } })
  } catch (cause) {
