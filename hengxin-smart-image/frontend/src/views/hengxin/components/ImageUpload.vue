<template>
  <div class="image-upload">
    <ElUpload :key="uploadGeneration" ref="upload" :drag="!sortable" :multiple="maxCount !== 1" accept="image/png,image/jpeg,image/webp"
    ...
    <ElButton v-if="isMockMode && !hideExamples" text type="primary" :disabled="disabled" :loading="examplesLoading" @click="example">
      {{ exampleLabel || '使用示例素材' }}
    </ElButton>
    <p class="hx-footnote">{{ isMockMode ? `模拟接收，仅本次页面有效。${hideExamples ? '' : '示例按钮将替换当前图片。'}` : '图片上传后保存在服务器；移除仅取消当前选择。' }}
