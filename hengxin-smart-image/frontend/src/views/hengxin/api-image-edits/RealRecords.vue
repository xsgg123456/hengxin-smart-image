  ...
  <div class="hx-page">
    ...
    <ElCard class="art-card hx-section" shadow="never">
      ...
      <ArtTable :data="tasks" :columns="columns" :loading="loading" height="auto" :show-table-header="false" :show-pagination="false" empty-height="300px" :empty-text="error ? '列表加载失败，请重试' : '暂无换图记录'" row-key="id">
        ...
        <template #progress="{ row }"><span>{{ row.items.filter((item: ApiItem) => item.state === 'succeeded').length }} / {{ row.items.length }} 张成功</span><p class="hx-footnote">{{ row.items.filter((item: ApiItem) => item.state === 'failed').length }} 张失败 · 串行处理</p></template>
...
const columns = [{ prop: 'name', label: '任务', useSlot: true, minWidth: 260 }, { prop: 'status', label: '状态', useSlot: true, width: 120 }, { prop: 'progress', label: '图片进度', useSlot: true, minWidth: 165 }, { prop: 'created', label: '创建时间', minWidth: 165 }, { prop: 'action', label: '操作', useSlot: true, width: 175 }]
