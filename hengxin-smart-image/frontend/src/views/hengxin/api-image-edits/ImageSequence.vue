<template>
  <div>
    ...
    <div v-if="images.length" class="sequence-grid hx-gap">
      <div v-for="(picture, index) in images" :key="picture.url + index" class="sequence-card" :class="{ material: index === images.length - 1 }" draggable="true" @dragstart="dragIndex = index" @dragover.prevent @drop.prevent="drop(index)">
...
function addFile(file: UploadFile) {
  const raw = file.raw
  if (!raw) return
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(raw.type) || raw.size > 10 * 1024 * 1024) { error.value = '请选择不超过 10 MiB 的 JPG、PNG 或 WebP 图片。'; return }
