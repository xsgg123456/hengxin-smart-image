<template>
  <div>
    ...
    <div v-if="state.images.length" class="sequence-grid hx-gap">
      <div v-for="(picture, index) in state.images" :key="picture.id" class="sequence-card" :class="{ material: index === state.images.length - 1 }" :draggable="!draft.locked()" @dragstart="dragIndex = index" @dragover.prevent @drop.prevent="drop(index)">
        ...
        <div class="sequence-actions"><ElButton size="small" :disabled="draft.locked() || index === 0" @click="draft.move(index, index - 1)">前移</ElButton><ElButton size="small" :disabled="draft.locked() || index === state.images.length - 1" @click="draft.move(index, index + 1)">后移</ElButton></div>
        <div class="sequence-actions"><ElButton text size="small" type="primary" :disabled="draft.locked() || index === state.images.length - 1" @click="draft.move(index, state.images.length - 1)">设为素材</ElButton><ElButton v-if="picture.state === 'failed'" text size="small" :disabled="draft.locked()" @click="draft.retry(picture)">重传</ElButton><ElButton text size="small" type="danger" :disabled="draft.locked() || ['uploading', 'removing'].includes(picture.state)" @click="draft.remove(picture)">移除</ElButton></div>
      </div>
    </div>
...
const { draft } = defineProps<{ disabled: boolean; draft: ReturnType<typeof createDraft> }>()
...
function addFile(file: UploadFile) { if (file.raw) draft.add(file.raw) }
function drop(to: number) { if (dragIndex.value !== null) draft.move(dragIndex.value, to); dragIndex.value = null }
</script>
<style scoped>
