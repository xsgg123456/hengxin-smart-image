...
import { getService, isMockMode, isDemoMode } from './hengxin/client'
...
export async function getExamplePictures(mode: Mode, count: number) {
  ...
  const { sampleImages, demoImages } = await import('./hengxin/fixtures')
  return (isDemoMode ? demoImages : sampleImages)(mode, count)
}
