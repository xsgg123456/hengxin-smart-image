...
export function createSkillManagement(db: Workspace, skills: SkillVersion[], check: (admin?: boolean, operation?: string) => Promise<User>, options: { scenario?: ManagementScenario; installMs?: number; snapshot?: DemoState; capture?: (state: Partial<DemoState>) => void }): Pick<ManagementService, 'listManagedSkills' | 'registerSkill' | 'checkSkill' | 'removeSkill' | 'setSkillStatus'> {
  const metadata = new Map<string, Omit<ManagedSkill, keyof SkillVersion>>(options.snapshot?.skillMetadata)
  const jobs = new Map<string, number>(options.snapshot?.skillJobs)
  const failedOnce = new Set<string>(options.snapshot?.skillFailedOnce)
  const capture = () => options.capture?.({ skillMetadata: [...metadata], skillJobs: [...jobs], skillFailedOnce: [...failedOnce] })
  ...
  return {
    async listManagedSkills() { await check(true); finish(); const result = options.scenario === 'empty' ? [] : skills.map(managed); capture(); return result },
    async registerSkill({ name, mode, version, description }) {
      ...
      skills.unshift(skill); const result = managed(skill); capture(); return result
    },
    async checkSkill(id) {
      ...
      skill.status = 'checking'; metadata.set(id, { ...managed(skill), error: null, updatedAt: new Date().toISOString() }); jobs.set(id, Date.now() + (options.installMs ?? 1200)); const result = managed(skill); capture(); return result
    },
    async removeSkill(id) {
      ...
      skills.splice(skills.indexOf(skill), 1); metadata.delete(id); capture()
    },
    async setSkillStatus(id, status) {
      ...
      skill.status = status; metadata.set(id, { ...managed(skill), updatedAt: new Date().toISOString() }); const result = managed(skill); capture(); return result
    }
  }
}
