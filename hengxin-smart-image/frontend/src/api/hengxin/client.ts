...
// 只有显式 mock / demo 模式加载模拟模块；生产构建采用真实 HTTP，不回退。
export const isDemoMode = import.meta.env?.MODE === 'demo'
export const isMockMode = import.meta.env?.MODE === 'mock' || isDemoMode
let service: Promise<HengxinService> | undefined
if (import.meta.hot) import.meta.hot.dispose(() => { void service?.then(value => value.dispose()).catch(() => undefined); service = undefined })
export function getService(): Promise<HengxinService> {
  service ??= isMockMode
    ? import('./mock').then(({ createMockService }) => {
      ...
      return isDemoMode
        ? import('./demo-service').then(({ createDemoService }) => createDemoService({ scenario, managementScenario }))
        : createMockService({ scenario, managementScenario })
    })
  ...
  return isDemoMode ? service.catch(reason => { service = undefined; throw reason }) : service
}
